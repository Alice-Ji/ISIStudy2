...
// -----------------------------
function saveState() {
  localStorage.setItem("auv_N_state", JSON.stringify({
    posts,
    collectedComments,
    shopNowClicks
  }));
...
function loadState() {
  const saved = localStorage.getItem("auv_N_state");
  if (saved) {
...
let posts = [
  {
    ...
    media: [
      "https://raw.githubusercontent.com/Alice-Ji/ISIStudy2/main/auv_N.mp4",
    ],
